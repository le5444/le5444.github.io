...
公开项目入口保持为 **织梦写作台 / Zhimeng Writing Agent**。底层运行层为 **灵枢 LumenOS**：它让织梦不只是提示词网页或单点小说编辑器，而是把上下文记忆、Skills、Tool Use、项目管理、长期记忆、模型 Provider、Worker、MCP/Gateway、安全审批和多工作区组织成一个可持续运行的写作 Agent 工作台。
...
12. 默认 `SYSTEM_PROMPT` 已校准为“织梦写作台的长期写作 Agent，底层由灵枢 LumenOS 提供 Personal Agent OS 能力”；writing 保持默认主场，coding/research/automation 等任务按需挂载对应 Domain Agent。
...
65. 主外壳品牌纠偏已落地并再次校准：浏览器标题、Open Graph、README 首屏和公开入口保持 **织梦写作台 / Zhimeng Writing Agent**，`AgentControlCenter` 说明 LumenOS 是底层 Agent OS / Agent IDE；`Activity Bar` 补齐模型 Provider、Worker、规格 / 钩子、写作 Agent 等核心 View Container。这样用户从 GitHub Pages 或 source 分支进入时，第一印象仍是写小说 Agent，深入后能看到支撑它长期运行的 Agent IDE 底盘。
...
77. Provider / Worker 记录层脱敏已补齐：Gateway `save_record` 现在会在写入 `bridge/runs` 与 `bridge/approvals` 前递归清理 `api_key` / `apiKey` / `Authorization` / `x-api-key` / `token` / `secret` / `cookie` / `password` 等敏感字段，并清理 URL query 中的 `key` / `token` / `secret`。Codex2API 仍按 OpenAI-compatible `GET /models` 拉取模型列表；探针结果、请求头和远端响应文本都会脱敏，key 不写入源码、文档、运行记录或构建产物。
...
84. 设置弹窗模型发现入口已落地：`AI 模型设置` 里新增“模型发现”，用户只填 base URL 和 API key 即可通过 Gateway `provider_probe` 读取 OpenAI-compatible `/models`，结果可一键填入模型 ID 或保存为本地配置档案，体验更接近 cc-switch 的 base URL / key / model 管理流。该请求默认带 `record=false`，不写 `bridge/runs`；后端 `provider_probe` 现在还会对返回 URL、请求头和远端响应文本做二次脱敏，避免一次性 key 进入 UI、运行记录或构建产物。当前本机对 Codex2API 的真实探针路径已验证到 Gateway 和脱敏层：无 key 返回 401 API_KEY_REQUIRED，带一次性 key 返回 403 Forbidden，说明端点可达但该 key 未通过服务端授权。
85. 底部 Worker 任务时间线首版已落地：Bottom Panel 新增“时间线”标签，将 `worker_status.recent_jobs`、`worker_status.recent_events` 与 `runtime_events` 中的 Worker 来源合并成按 job 聚合的只读时间线，展示任务数、活跃数、问题数、事件来源、job id、阶段、状态和详情；导出当前 Panel 日志时也会把时间线转换成 runtime log 行。该视图不执行命令、不调用模型、不批准审批，只把 Worker / Gateway / 事件流串成更接近 Codex / Claude Code 的运行审计面板。
86. Provider 探针错误诊断已补齐：设置弹窗与 Provider 中枢都会把 401/403 显示为 API key 缺失、格式不被接受、权限/额度/白名单/账号绑定问题；404/405 显示为 `/models` 路径不可用并建议回退手填模型 ID；network_error 显示为 URL、代理、防火墙或 TLS 问题。公开 cc-switch 文档同样把 Fetch Models 建立在 API Key + Endpoint + OpenAI-compatible `/v1/models` 之上，并提示 401/403 查 key、404/405 回退手填；OpenAI 官方 API 也提供 models list 端点。
87. Provider 模型能力选择器首版已落地：设置弹窗和主工作台 Provider 中枢都会对 `/models` 返回的模型 ID / display_name / owned_by 做轻量能力标签推断，展示 `编码`、`推理`、`长上下文`、`多模态`、`低延迟`、`本地`、`聚合` 等标签，并支持按模型 ID / provider / 能力搜索和按标签筛选。模型发现历史也会保存这些脱敏标签，便于复用上次探针结果；标签只用于辅助选择，不保存 API key、不调用模型生成、不绕过 Gateway Provider gate。
88. Agent 线程任务状态协议 / Runbook 首版已落地并接入载荷：主工作区会从当前线程消息、工具轨迹、Worker、审批、Diff、上下文附件和 Provider/Gateway 状态动态归纳任务阶段、下一步动作、阻塞点、五阶段进度和证据源。Runbook 会以 `agent_thread_runbook` 自动注入 `thread_context`，进入后续 `context_pack`、只读 Worker 和模型 Worker 预检载荷；用户也可一键“挂 Runbook”把当前状态固化为线程附件。它让线程更接近 Codex / Claude Code 的可恢复工程任务，而不是普通聊天记录；该协议只归纳现有轨迹和导航审查，不自动执行工具、不批准审批、不写文件。
89. Agent 指令栈 / Instruction Stack 首版已落地：Workbench 新增“指令栈”入口，Activity Bar、主侧边栏、命令面板和主工作区都会展示 Codex `AGENTS.md`、Claude Code `CLAUDE.md`、Kiro `Steering / Specs / Hooks`、灵枢产品边界、公开源码边界、线程 Runbook、工作区 Skills 和 Gateway 安全闸门的层级关系。`instruction_stack` 会自动注入 `thread_context`，并在 `thread_context_policy.uses` 中与 `agent_thread_runbook`、`thread_context`、`specs`、`steering`、`hooks` 一起声明；工作区级 `context_pack` 也同步声明该协议。Specs 草案会写入“指令栈”章节，确保这不是单纯 UI 卡片，而是后续 Agent / Worker 可审查的任务规则层。当前指令栈只读注入，不自动创建 `AGENTS.md` / `CLAUDE.md` / `.lumen/*`，不批准审批，不复制或移植泄露源码。
90. 项目规则文件只读同步首版已接入 Instruction Stack：指令栈面板新增“同步指令栈”，会通过 Gateway `read_file` 只读读取 `AGENTS.md`、`CLAUDE.md`、`.lumen/steering/lumenos.md`、`.lumen/specs/current/{requirements,design,tasks}.md` 和 `.lumen/hooks/lumenos-hooks.md`，读取成功后把内容压成短摘要，而不是全文塞进上下文。同步结果会显示逐文件状态，并作为 `instruction` 类型附件进入 `instruction_stack` / `thread_context`；缺失文件只显示 missing，不创建文件、不写入、不批准审批、不扫描全盘。这一步把 Codex / Claude Code / Kiro 的项目规则从“概念展示”推进到真实可审查上下文来源。
91. 分层项目规则发现首版已接入 Instruction Stack：同步指令栈现在会优先用当前工作区根目录执行 `workspace_scan` 元数据扫描，发现嵌套 `AGENTS.md`、`CLAUDE.md`、`.lumen/steering/*`、`.lumen/specs/*` 和 `.lumen/hooks/*`，再通过 `read_file` 只读读取摘要；没有根目录或读闸门不足时回退到固定项目根规则列表。UI 会展示工作区、根目录、发现数、逐文件 scope、inclusion mode（always / manual / fileMatch / event-bound）、发现状态、大小和摘要；同步成功的每个规则文件也会成为 `instruction_stack` 的独立层，进入 `thread_context` 与后续 `context_pack`。该扫描只读取路径元数据和用户显式同步的规则正文，不扫描全盘、不创建规则文件、不写文件、不批准审批。
92. 指令栈规则初始化草案与写入审批已落地：指令栈面板新增“生成规则草案”和“提交规则审批”，可生成 `AGENTS.md`、`CLAUDE.md`、`.lumen/steering/lumenos.md`、`.lumen/specs/current/{requirements,design,tasks}.md`、`.lumen/hooks/lumenos-hooks.md` 七个中文优先项目规则文件草案。草案会明确 LumenOS 顶层产品边界、Codex / Claude Code / VS Code Workbench 对标方向、公开资料边界、Phase 1-5、thread_context/context_pack、Skills、Gateway 安全门和 Hooks 草案；提交时逐个调用 Gateway `write_file` 且不带 `execute=true`，因此只进入审批队列，不直接创建文件、不改写项目、不绕过 `write_file` approval。规则草案文件会展示 scope、inclusion mode、kind 和内容长度，并把返回的 approval id 挂回当前 Agent 线程。
93. 指令栈规则作用域审计首版已落地：指令栈面板新增“规则作用域审计”，会把已同步或草案中的 `AGENTS.md`、`CLAUDE.md`、Steering、Specs、Hooks 归纳成来源边界、AGENTS 作用域、CLAUDE 项目记忆、Steering/Specs、Hooks 边界、缺失初始化、路径冲突等审计行。每行展示 status、severity、证据路径和下一步动作，能提示缺失规则需要生成草案、嵌套 AGENTS.md 会按最近路径优先、Hooks 仍是 event-bound 草案、重复路径会阻塞注入质量。这一步把 Codex / Claude Code / Kiro 的规则层从“能同步”推进到“能解释谁生效、谁缺失、谁冲突”；仍不执行 Hook、不写文件、不批准审批。
94. 当前目标规则匹配解释首版已落地：指令栈面板新增“当前目标规则匹配”，会依据当前 Editor 文件标签、Diff 标签、workspace_scan 选中路径或当前 Agent 线程任务，解释哪些 `AGENTS.md`、`CLAUDE.md`、Steering、Specs、Hooks 会进入上下文。Codex 风格的 AGENTS.md 会按目录作用域和最近路径优先解释，较远 AGENTS 会被标记为 shadowed；CLAUDE.md 作为项目记忆默认随线程进入；Steering 根据 always/manual/fileMatch 解释，Specs 作为任务上下文进入计划审查，Hooks 保持 event-bound 不直接执行。该能力只做本地匹配说明，不读文件、不写文件、不触发 Hook、不批准审批。
95. 当前目标规则匹配已注入 `thread_context/context_pack`：规则匹配结果现在会生成独立 `instruction_rule_match` 上下文附件，与 `instruction_stack`、`agent_thread_runbook`、线程附件一起进入命令中心、只读 Worker、模型 Worker 预检和工作区级 context_pack 的 `thread_context_policy.uses`。指令栈面板的“注入协议”会明示 `instruction_rule_match=auto-injected-current-target-scope`，避免它只是 UI 解释卡片；同时保留 Runbook 注入，不会被规则匹配覆盖。该协议仍是只读摘要，不自动读取额外正文、不执行 Hook、不写文件、不批准审批。
96. 线程级上下文注入历史首版已落地：命令中心每次生成任务草案时，会把本次 `thread_context/context_pack` 的注入结果保存为本地 `ThreadContextSnapshot`，记录线程、工作区、任务、`thread_context_policy.uses`、规则/Runbook/线程附件来源、Gateway 上下文切片、active_skill_keys 和工具排除项。Agent 运行台新增“上下文注入历史”面板，展示最近快照、相对上一次新增/移除的注入项、来源计数，并可把历史快照重新挂入当前线程。该历史只保存在浏览器本地摘要中，不写项目文件、不上传、不执行工具、不批准审批；它让任务恢复更接近 Codex / Claude Code 的可回放工程上下文。
97. 上下文快照 Diff 明细首版已落地：`ThreadContextSnapshot` 对比现在不只给新增/移除数量，还会把 `thread_context` 和 Gateway `context_pack` 切片合并成可比较条目，按来源、kind、标题生成 `ContextSnapshotDiffItem`。Agent 运行台的“上下文注入历史”每条快照会展示“新增注入 / 移除注入”明细，能看见是哪条规则、Runbook、线程附件或上下文切片发生变化。该 Diff 只比较本地摘要 key，不读取额外文件、不展开全文、不执行工具；后续可继续补按来源筛选、固定基线和导出对比报告。
98. 上下文快照固定基线与导出报告首版已落地：Agent 运行台的“上下文注入历史”现在可以把任意 `ThreadContextSnapshot` 固定为当前线程的对比基线，后续快照优先与该基线比较；也可导出本地 Markdown 对比报告，报告包含当前/基线任务、policy uses、来源计数、新增注入、移除注入和安全边界。基线映射只存在浏览器 localStorage，导出只触发浏览器下载，不写项目文件、不调用 Gateway、不读取额外正文、不执行工具；下一步可补来源筛选和报告模板细化。
99. 上下文快照来源/分组筛选首版已落地：Agent 运行台的“上下文注入历史”新增来源筛选与分组筛选，可按 `instruction_stack`、`instruction_rule_match`、`agent_thread_runbook`、`workspace_context_pack` 等注入来源，或按 `thread_context` / `context_pack` 分组查看新增/移除明细。顶部 delta、每条快照 Diff 明细和 Markdown 导出报告都会使用当前筛选条件，报告中也会写入“来源筛选 / 分组筛选”。筛选只作用于浏览器本地摘要视图，不改变原始快照、不影响 Worker / Gateway payload、不批准审批。
100. 审批执行器预检首版已落地：审批复核台新增“专用审批执行器”卡片，会按当前 approval action 解释 `write_file`、Memory 管理或 `provider_probe` 使用哪个执行器、需要哪个 Gateway flag（`--execute-write` / `--execute-memory` / `--execute-provider`）、请求门是什么，以及当前 runtime capability 是否允许执行。执行按钮现在不仅看 action 白名单，还会检查 Gateway 在线、审批未终态、没有决策在运行、对应执行门已开启；不满足时显示阻塞原因。`provider_probe` 的执行决策分支也已补齐，避免 UI 显示可执行但 `approval_decide` 前端函数拒绝派发。该预检仍只调用现有 `approval_decide`，不新增任意 shell、不绕过远程模型 `allow_remote_model`、不扩大审批白名单。
101. 审批 Editor Tab 首版已落地：`EditorTabKind` 新增 `approval`，审批队列、线程关联审批和命令面板 `/approval.open` 都能把单条 approval 打开为主工作区 Editor Group 标签。审批标签展示动作、状态、目标、时间、专用执行器预检、`Proposal / Request / Result / Decision` 详情 JSON，并复用现有“关联当前线程 / 拒绝审批 / 执行审批”按钮；执行仍走 `approval_decide`、Gateway flag、请求级 `execute=true` 与远程模型授权，不新增执行通道。这个能力把 approvals 从底部 Panel 里的列表提升为可像 Diff 一样审查、切换和持久化的工作台对象，更接近 Codex / Claude Code / VS Code 的任务审查流。
102. Worker Editor Tab 首版已落地：`EditorTabKind` 新增 `worker`，底部 Worker 时间线、最近 Worker 列表和命令面板 `/worker.open` 可把单个 Worker job 打开为主工作区 Editor Group 标签。Worker 标签按 job id 聚合 `worker_status.recent_jobs`、`recent_events`、`runtime_events` 和本地 runtime log，展示状态、时间线、payload 请求、result 结果和匹配日志，并明确安全边界：只读审查，不执行命令、不调用模型、不批准审批；模型 Worker 仍受 Provider 配置、`execute_model`、远程授权和 Gateway 执行门控制。这样 Worker 不再只是底部 Panel 里的运行日志，而是能像 Diff / Approval 一样成为可恢复、可切换、可追踪的 Agent IDE 工作台对象。
103. Agent 线程 Tool Trace 深链首版已落地：任务轨迹面板现在不只是展示消息、工具、审批、Diff 和 Worker 摘要，还会把对应行直接深链到 Editor Group 标签。审批轨迹可“打开审批标签”，Diff 轨迹继续打开 Diff 标签，Worker 轨迹可“打开 Worker 标签”；缺少实时记录时会用线程快照生成只读标签并提示刷新。该能力把 Agent 线程从普通聊天/日志列表推进到 Codex / Claude Code 风格的可回放任务时间线：每个关键行动都能跳到审查对象，但仍只导航和审查，不新增执行通道、不批准审批、不写文件。
104. Agent 线程运行报告导出首版已落地：线程管理器和“上下文注入历史”现在都能导出当前 Agent 线程运行报告 Markdown，报告会把任务摘要、Runbook 阶段、下一步、阻塞点、Tool Trace、关联审批、Changes / Diff、Worker 时间线、thread_context/context_pack 快照、上下文附件和最近 runtime log 收束为一份可交接文档。该导出只使用浏览器本地状态，只触发 Markdown 下载，不调用 Gateway、不访问远程模型、不执行 Worker/终端、不批准审批、不写项目文件；它补上了 Codex / Claude Code 式任务恢复与人工交接需要的“run report”层，区别于单独的上下文快照 Diff 报告。
105. Agent 运行报告 Artifact / Editor Tab 首版已落地：运行报告不再只是一次性下载文件，前端新增 `lumenos-agent-run-reports` 本地历史和 `run_report` Editor Tab 类型；生成报告会同时下载 Markdown、保存本地 artifact、打开主工作区报告标签。报告标签展示 Markdown 源文、阶段、Tool Trace、审批、Diff、Worker、上下文快照数量、Thread ID、工作区和安全边界，并可再次下载；命令面板新增 `/report.open`，线程管理器新增“运行报告 Artifacts”列表，可像 Diff / Approval / Worker 一样重新打开最近报告。该能力只读取浏览器本地报告历史，不调用 Gateway、不访问模型、不执行 Worker、不批准审批、不写项目文件，把 Agent 任务交接推进到 VS Code Editor Group + Claude/Codex Artifact 风格。
106. Agent 运行报告 Artifact 挂入 `thread_context` 首版已落地：运行报告标签、线程管理器 Artifacts 列表和命令面板 `/report.attach` 都能把最近或当前报告挂入当前 Agent 线程；挂入内容只包含阶段、Tool Trace、审批、Diff、Worker、上下文快照数量和任务摘要，完整 Markdown 仍留在本地 artifact，不进入上下文载荷。`thread_context_policy.uses` 已声明 `agent_run_report`，后续 `context_pack`、只读 Worker 和模型 Worker 预检可识别这类交接摘要；该动作不调用 Gateway、不访问模型、不执行 Worker、不批准审批、不写项目文件，只增强任务恢复和人工交接的上下文索引。
107. Agent 线程 Checkpoint 分支协议首版已落地：线程管理器和消息流现在把“创建分支 / 从此创建分支”升级为非破坏性 checkpoint 分支，分支会记录父线程、检查点消息、继承消息数、上下文附件数和审批引用数，并把 `Agent Thread Branch` 摘要作为 `branch` 类型上下文附件挂入新线程。`thread_context_policy.uses` 已声明 `agent_thread_branch`，导出线程 Markdown 会新增“分支来源”章节，后续 `context_pack`、只读 Worker 和模型 Worker 预检能识别当前任务是从哪个父线程岔出；该动作只复制浏览器本地线程摘要，不修改父线程、不调用 Gateway、不执行工具、不批准审批、不写项目文件。
108. Agent Home 专注模式首版已落地：`Agent OS 控制台` 现在采用更接近 Codex / Claude Code 任务入口与 VS Code Workbench 的轻量三栏：左侧只放线程搜索和空间筛选，中间聚焦当前任务输入、Runbook 阶段、消息流和附件发送，右侧把上下文、审批、产物、轨迹收束为可切换辅助面板。外层主侧栏、辅助侧栏、底部 Panel 和运行时检查器在 Agent 首页默认不再叠加显示，避免“双层工作台”和信息墙；切到工作区、Provider、Diff、Approval、Worker 等高级标签时仍保留完整 VS Code 式工作台。该改动只调整前端信息架构与本地布局状态，不调用 Gateway、不访问模型、不执行 Worker、不批准审批、不写项目文件。
109. Agent Home 关键入口可发现性已补强：首屏左侧和顶部动作区新增明确的 `AI 对话` 与 `API 配置` 入口，`AI 对话` 会直接聚焦线程输入框，`API 配置` 直达模型设置弹窗，旁边保留 `Provider 中枢` 进入完整模型管理页；消息区标题、空状态、输入占位和发送按钮统一改为中文优先的对话语义。这样首页仍保持 Codex / Claude Code / VS Code Workbench 式三栏骨架，但第一眼能看见“开始对话”和“配置 API / 模型”的主路径；公开资料和开源项目只用于交互结构参考，不复制或移植泄露源码。
110. Agent Home Chat-first 重排已落地：参考公开的 Claude Code UI / CloudCLI、Crest / Codex 风格 Web IDE、QClaw / OpenClaw 类 Agent 面板的信息架构后，首页进一步收敛为“左侧会话与工作区、中央 AI 对话、右侧上下文/审批/产物/轨迹”。中间不再同时堆 Runbook、目标任务大输入和系统检查器；当前目标变成一行命令输入，Runbook、计划草案、context_pack 标记收进右侧上下文区。响应式布局改为中等宽度即保持左会话栏 + 中央对话，右侧上下文只在宽屏出现，避免窄窗口首屏先看到纵向堆叠的抽象面板。
111. Agent Home 顶部专注态已落地：在 Agent 首页隐藏 `Activity / 侧栏 / 辅助 / Panel / 状态栏 / 重置` 这类 Workbench 调试按钮和 `Editor Group / 主工作区` 标签栏，只保留 `新对话`、`命令`、`API 配置` 等用户可理解动作；底部状态栏保留 Gateway、模型、审批、Worker、观察等运行状态，隐藏 `Parts` 与 `重置布局`。进入 Provider、工作区、Diff、Approval、Worker 等高级视图时仍恢复完整 VS Code 式 Workbench 控制，Agent Home 则更接近 Codex / Claude Code 的会话入口。

后续优先补：

1. 继续把 `Agent 线程` 从本地会话系统升级到真实任务协议：线程搜索/空间过滤/导出/删除/分支、上下文附件、审批关联、context_pack 协议注入、模型 Worker 请求注入、事件流 UI、流式回复原地更新、审批状态刷新同步、运行观察轻轮询、增量事件游标、SSE 长连接事件流、跨工作区 thread spaces 存储、write_file 授权执行闭环、Memory 授权执行闭环、Provider probe 授权执行闭环、Runbook、Instruction Stack、项目规则文件只读同步、分层规则发现、规则初始化审批、规则作用域审计、当前目标规则匹配解释、`instruction_rule_match` 注入、线程级上下文注入历史、上下文快照 Diff 明细、固定基线、上下文快照报告、来源/分组筛选、审批执行器预检、审批 Editor Tab、Worker Editor Tab、Tool Trace 深链、Agent 运行报告、运行报告 Artifact / Editor Tab、运行报告挂入 `thread_context`、Checkpoint 分支协议和 Agent Home 专注模式首版已落地，下一步是多项目 thread space 持久化协议、任务状态协议收敛、更多执行器深链、报告模板细化和首页右栏键盘化切换。
2. 继续把 `Changes / Diff` 从单文件派生面板升级成完整多文件 diff 系统：多文件 proposal、真实文件路径映射、hunk 解释、回滚草案、历史 diff、与审批队列双向同步。
3. 继续把 `底部 Panel` 从本地 runtime log 流升级成真实可审查运行面板：allowlist 验证命令闭环、运行观察轻轮询、增量事件游标、SSE 长连接订阅、Worker/Gateway/审批统一事件流和 Worker 任务时间线首版已落地，下一步是历史命令索引增强、stdout/stderr 更细分流、时间线分组折叠和任务详情深链。
...
6. Provider/API 设置中心已具备配置草案、脱敏 payload、profile 持久化编辑/激活/删除、设置弹窗模型发现、只读状态检查、探针审批草案、线程附件、`provider_probe` 审批执行门、Codex2API `/models` 拉取、模型能力标签/筛选、模型 Worker 测试闭环和运行观察同步首版；下一步补更多 provider wire format 适配、探针历史对比和长连接式审批状态流。
