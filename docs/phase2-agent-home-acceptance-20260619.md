...
| 右侧辅助栏 | 只保留上下文、文件、变更、审批、状态，不出现模型/API 配置标签；折叠态也必须显示极短中文标签，不能只靠图标猜；展开后顶部必须有可见的“当前下一步”“回到对话”和“收起侧栏”出口 | `npm run verify:agent-home-sidebar` |
...
5. 右侧辅助栏默认是窄工具栏，并用 `data-panel-state="collapsed"` 暴露状态；折叠态仍显示“上下文 / 文件 / 变更 / 审批 / 状态”短标签，展开详情后要能直接看到“当前下一步”，并能点“回到对话”或“收起侧栏”，不把调试卡片、API 配置和运行日志堆满首屏。
