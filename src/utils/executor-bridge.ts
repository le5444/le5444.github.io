...
export const DEFAULT_EXECUTOR_BRIDGE: ExecutorBridgeManifest = {
  ...
  safety: [
    ...
    "write_file 默认先进入前端 Changes / Diff 审查；一个回复可以输出多个 write_file 请求，或在 payload.files[] 中放多个 {path, content, mode} 文件草案。用户接受后每个文件分别进入 approval-draft；只有 Gateway 显式 --execute-write 且 payload.execute=true 时，才允许工作区写入，并自动备份。",
    ...
    "provider_catalog/provider_config_status/provider_status 只读模型供应商注册表与本机配置；provider_probe 默认只生成审批草案，审批后执行仍需要 Gateway --execute-provider、payload.execute=true，远程端点还需要 allow_remote_model=true。",
...
export function extractExecutorBridgeRequestsFromText(text: string, manifest: ExecutorBridgeManifest = DEFAULT_EXECUTOR_BRIDGE): ExecutorBridgeRequest[] {
  const matches = [...(text || "").matchAll(/<bridge-request\b[^>]*>([\s\S]*?)<\/bridge-request>/gi)];
  return matches
    .flatMap((match) => {
      try {
...
8. provider_catalog/provider_config_status/provider_status 是只读 Provider 注册表与本机配置；provider_probe 只能探测模型列表端点，执行已排队探针仍需要 --execute-provider + execute=true，且远程探测必须显式 allow_remote_model=true。
...
11. 多文件写入优先使用一个 write_file 请求的 payload.files 数组，例如 {"files":[{"path":"a.md","mode":"replace","content":"..."},{"path":"b.md","mode":"append","content":"..."}]}；前端会把每个文件拆成独立 Diff、独立 hunk 审查和独立 write_file 审批。
12. 当你需要本地工具时，输出以下 JSON 标签，除此之外不要伪造执行结果：
